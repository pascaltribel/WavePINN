...
vp = np.load("marmousi_vp.npy")
vp_normalized = vp / vp.mean() * 4 # normalize to be physically consistent with c=10

x_coords = np.linspace(-10, 10, vp.shape[0]) 
y_coords = np.linspace(-10, 10, vp.shape[1])

interp_marmousi = RegularGridInterpolator(
    (x_coords, y_coords), vp_normalized,
...
def c(x, y, t=0):
    ...
    return torch.tensor(interp_marmousi(np.stack([x.ravel(), y.ravel()], axis=-1)).reshape(x.shape), dtype=torch.float32).to(device)
...
Z = c(X, Y)
plt.imshow(Z.cpu().numpy(), extent=[-5, 5, -5, 5], origin='lower', aspect='auto', cmap='viridis')
...
def run_fd_reference(c0: float = 1.0, t_max: float = 5.0, n_t: int = 800, L: float = 5.0, nx: int = 100, t0=1, cx=0, cy=0):
    dx = L*2/nx
    dt = t_max / n_t
    #nx = int(2 * L / dx) + 1
...
plt.plot(run_fd_reference()[1]["(-1.2,3)"])
...
receiver_labels=["(-1.2,3)", "(0.1,-0.4)"]

def check_seismograms(model=None, c_val: float = 10.0, t_max: float = 5.0, n_t: int = 800, t_0=[1.], cx=0, cy=0):
    if model == None:
    ...
    for i in range(len(t_0)):
        ...
        with torch.no_grad():
            ...
            pinn_seis = {'(-1.2,3)': p_neg1, '(0.1,-0.4)': p_zero}
...
if TRAINING:
    ...
    SPATIAL_RANGE_X  = 5.0
    SPATIAL_RANGE_Y  = 5.0
    SPATIAL_RANGE_X0 = 4.0
    SPATIAL_RANGE_Y0 = 4.0
    T0_MAX        = 3.0
    T_MAX         = 6.0
    N_STEPS       = 100_000
    LR            = 5e-3
    ...
    def sample_batch():
        ...
        t0 = torch.rand(N_POINTS, 1, device=device)*T0_MAX
    ...
    for step in (pbar := tqdm(range(N_STEPS))):
        ...
        if loss_pde.item() < min_loss:
            ...
            torch.save(model, "model_marmousi.pt")
            check_seismograms()
else:
    model = torch.load("model_marmousi.pt", weights_only=False)
...
def run_fd_reference_with_fields(t_max: float = 5.0, n_t: int = 800,
                                  L: float = 5.0, nx: float = 100, t0=1, cx=0, cy=0,
...
def check_wave_field(model, t_max: float = 5.0,
                     n_t: int = 800, t_0=1, cx=0, cy=0,
                     L: float = 5.0, nx: float = 100,
                     snapshot_every: int = 10):
    field_gif_frames = []
...
check_wave_field(model, t_max=3, n_t=400, cx=0.0, snapshot_every=5)
